...
async function loadAllData() {
  ...
  allPlayers = users.filter(u => u.role === 'player' && u.active !== false).sort((a,b) => (a.jersey_number||0) - (b.jersey_number||0));
...
function renderOverview() {
  ...
  const nutritionDoneCount = todayConditions.filter(c => c.nutrition_done).length;
  const painCount = todayConditions.filter(c => c.has_pain).length;
  const totalPlayers = allPlayers.length;
  const rehabCount = allPlayers.filter(p => p.rehab_phase_id).length;
  ...
  el.innerHTML = `
    <h1 class="font-display text-2xl font-bold mb-1">概要サマリー</h1>
    <p class="text-gray-400 text-sm mb-6">${today} の全体状況</p>

    <div class="grid grid-cols-4 gap-4 mb-8">
      ${statCard('fa-users', totalPlayers, '登録選手数', 'text-blue-400')}
      ${statCard('fa-drumstick-bite', `${nutritionDoneCount}/${totalPlayers}`, '本日の補食実施', 'text-green-400')}
    ...
    <div class="grid grid-cols-2 gap-6">
      ...
      <div class="bg-[#1f2229] rounded-2xl p-5">
        <h2 class="text-sm font-semibold text-gray-300 mb-4"><i class="fa-solid fa-chart-simple mr-1 text-[var(--kuki-red)]"></i>本日のトレーニング進捗</h2>
        <div style="height: 220px;"><canvas id="progress-canvas"></canvas></div>
      </div>
    </div>
  `;
...
function statCard(icon, value, label, colorCls) {
  return `
    <div class="bg-[#1f2229] rounded-2xl p-5">
      <i class="fa-solid ${icon} text-2xl ${colorCls} mb-3"></i>
...
function renderAlertList() {
  ...
  const todayConditions = allConditions.filter(c => c.date === today);
  allPlayers.forEach(p => {
    const cond = todayConditions.find(c => c.user_id === p.id);
    if (cond && !cond.nutrition_done) {
      alerts.push({ type: '補食未実施', name: p.name, detail: '本日の補食未チェック', level: 'low' });
    }
  });

  if (alerts.length === 0) {
    return '<p class="text-gray-500 text-sm">現在、特に注意すべきアラートはありません</p>';
  }
  ...
  return `<div class="flex flex-col gap-2 max-h-64 overflow-y-auto no-scrollbar">
    ${alerts.slice(0, 15).map(a => `
      <div class="border-l-4 ${levelStyle[a.level]} rounded-r-lg px-3 py-2">
        <div class="flex items-center justify-between">
          ...
          <span class="text-[10px] px-2 py-0.5 rounded-full ${levelStyle[a.level]}">${a.type}</span>
        </div>
...
function drawProgressChart(done, remaining) {
  ...
  progressChart = new Chart(ctx, {
    ...
    data: {
      labels: ['完了', '未完了'],
      datasets: [{ data: [done, remaining], backgroundColor: ['#16a34a', '#374151'] }]
    },
    options: {
      responsive: true,
...
function renderPainView() {
  ...
  const painToday = allConditions.filter(c => c.date === today && c.has_pain);
  ...
  el.innerHTML = `
    ...
    <div class="bg-[#1f2229] rounded-2xl overflow-hidden">
      <table class="w-full text-sm">
        ...
        <tbody id="pain-history-body"></tbody>
      </table>
    </div>
  `;

  const bodyToday = document.getElementById('pain-table-body');
  const painRecordsToday = allPainRecords.filter(p => p.date === today);
  if (painRecordsToday.length === 0) {
    bodyToday.innerHTML = '<tr><td colspan="5" class="text-center text-gray-500 py-6">本日、痛みの報告はありません</td></tr>';
  } else {
    bodyToday.innerHTML = painRecordsToday.map(p => painRow(p)).join('');
  }

  const bodyHistory = document.getElementById('pain-history-body');
  const sorted = [...allPainRecords].sort((a,b) => new Date(b.date) - new Date(a.date));
  bodyHistory.innerHTML = sorted.length
    ? sorted.slice(0, 30).map(p => `
        <tr class="border-t border-gray-800">
          <td class="px-4 py-2.5">${userName(p.user_id)}</td>
    ...
    : '<tr><td colspan="4" class="text-center text-gray-500 py-6">記録がありません</td></tr>';
}

function painRow(p) {
  return `
    <tr class="border-t border-gray-800 alert-card">
      <td class="px-4 py-3 font-medium">${userName(p.user_id)}</td>
      <td class="px-4 py-3">${p.body_part}</td>
      <td class="px-4 py-3">${painLevelBadge(p.pain_level)}</td>
      <td class="px-4 py-3 text-gray-400">${p.note || '-'}</td>
      <td class="px-4 py-3 text-gray-400">${p.date}</td>
    </tr>
  `;
}

function painLevelBadge(level) {
  const colors = { 1: 'bg-yellow-500/20 text-yellow-300', 2: 'bg-amber-500/20 text-amber-300', 3: 'bg-orange-500/20 text-orange-300', 4: 'bg-red-500/20 text-red-300', 5: 'bg-red-700/30 text-red-400' };
  return `<span class="px-2 py-0.5 rounded-full text-xs font-semibold ${colors[level] || ''}">Lv.${level}</span>`;
}
  ...
  el.innerHTML = `
    ...
    <div class="grid grid-cols-3 gap-6">
      <div class="col-span-1 bg-[#1f2229] rounded-2xl p-5">
        <h2 class="text-sm font-semibold text-gray-300 mb-4">新規メニュー配信</h2>
        <div class="flex flex-col gap-3">
          <div>
            <label class="text-xs text-gray-400 block mb-1">種目名</label>
            <input id="menu-exercise" type="text" placeholder="例：バックスクワット" class="w-full bg-[#16181d] border border-gray-700 rounded-lg px-3 py-2 text-sm">
          </div>
          <div>
            <label class="text-xs text-gray-400 block mb-1">セット数</label>
            <input id="menu-sets" type="number" value="3" min="1" max="10" onchange="renderSetInputRows()" class="w-full bg-[#16181d] border border-gray-700 rounded-lg px-3 py-2 text-sm">
          </div>
          <div>
            <label class="text-xs text-gray-400 block mb-1">セットごとの強度（%）・回数</label>
            <p class="text-[11px] text-gray-500 mb-2">選手の1RMに対する%を各セットで設定できます（バックスクワットのように徐々に上げる場合など）</p>
            <div id="menu-set-rows" class="flex flex-col gap-2"></div>
          </div>
          <div>
            <label class="text-xs text-gray-400 block mb-1">配信対象グループ</label>
            <select id="menu-target" class="w-full bg-[#16181d] border border-gray-700 rounded-lg px-3 py-2 text-sm">
          ...
          <div id="menu-rehab-target-wrap" class="hidden">
            ...
            <select id="menu-rehab-target" class="w-full bg-[#16181d] border border-gray-700 rounded-lg px-3 py-2 text-sm">
              ${allPlayers.map(p => `<option value="${p.id}">${p.name}${p.rehab_phase_id ? '（リハビリ中）' : ''}</option>`).join('')}
            </select>
          </div>
          <div>
            <label class="text-xs text-gray-400 block mb-1">メモ</label>
            <textarea id="menu-note" rows="2" class="w-full bg-[#16181d] border border-gray-700 rounded-lg px-3 py-2 text-sm" placeholder="補足事項"></textarea>
          </div>
          <button onclick="distributeMenu()" class="w-full bg-[var(--kuki-red)] rounded-lg py-2.5 font-semibold text-sm mt-2"><i class="fa-solid fa-paper-plane mr-1"></i>配信する</button>
        </div>
      </div>

      <div class="col-span-2 bg-[#1f2229] rounded-2xl p-5">
        <h2 class="text-sm font-semibold text-gray-300 mb-4">配信済みメニュー一覧</h2>
        <div class="overflow-hidden rounded-xl border border-gray-800">
          <table class="w-full text-sm">
            <thead class="bg-[#16181d] text-gray-400 text-xs">
              <tr>
                <th class="text-left px-4 py-3">種目</th>
                <th class="text-left px-4 py-3">セット×回数</th>
...
// セット数に応じて「セットごとの強度(%)・回数」入力行を生成
function renderSetInputRows(prevIntensities, prevReps) {
  const setCount = Math.max(1, Math.min(10, parseInt(document.getElementById('menu-sets').value) || 1));
  ...
  for (let i = 0; i < setCount; i++) {
    ...
    rows += `
      <div class="flex items-center gap-1.5">
        ...
        <input type="number" class="menu-set-intensity w-0 flex-1 min-w-0 bg-[#16181d] border border-gray-700 rounded-lg px-2 py-1.5 text-sm" value="${intensityVal}" placeholder="強度%">
        <span class="text-[11px] text-gray-500 flex-shrink-0">%</span>
        <input type="number" class="menu-set-reps w-0 flex-1 min-w-0 bg-[#16181d] border border-gray-700 rounded-lg px-2 py-1.5 text-sm" value="${repsVal}" placeholder="回数">
...
function renderMenuTable() {
  ...
  body.innerHTML = sorted.map(m => {
    ...
    return `
    <tr class="border-t border-gray-800">
      <td class="px-4 py-3 font-medium">${m.exercise_name}</td>
      <td class="px-4 py-3">${m.sets} セット（各 ${repsDisplay} 回）</td>
      <td class="px-4 py-3 text-xs">${intensityDisplay}</td>
      <td class="px-4 py-3">
        <span class="px-2 py-0.5 rounded-full text-xs ${m.category === 'rehab' ? 'bg-amber-500/20 text-amber-300' : m.category === 'optin' ? 'bg-purple-500/20 text-purple-300' : 'bg-blue-500/20 text-blue-300'}">
          ${categoryLabel[m.category] || m.target_group}${m.category === 'rehab' ? '（' + userName(m.target_group) + '）' : ''}
        </span>
      </td>
...
async function distributeMenu() {
  ...
  try {
    ...
    alert('メニューを配信しました。セットごとの強度・回数で選手側に自動反映されます。');
  } catch (e) {
    console.error(e);
    alert('配信に失敗しました');
  }
...
function renderOptinView() {
  ...
  el.innerHTML = `
    ...
    <p class="text-gray-400 text-sm mb-6">選手からの希望メニューリクエストを承認・却下します</p>
...
function renderOptinCards() {
  ...
  if (sorted.length === 0) {
    el.innerHTML = '<p class="text-gray-500 text-sm col-span-3">リクエストはありません</p>';
    return;
  }
  const statusMap = {
    pending: { label: '承認待ち', cls: 'bg-yellow-600/20 text-yellow-400 border-yellow-600/40' },
    approved: { label: '承認済み', cls: 'bg-green-600/20 text-green-400 border-green-600/40' },
    rejected: { label: '却下', cls: 'bg-red-600/20 text-red-400 border-red-600/40' },
  };
  el.innerHTML = sorted.map(o => `
    <div class="bg-[#1f2229] border ${statusMap[o.status].cls} rounded-2xl p-4">
      <div class="flex items-center justify-between mb-2">
        <span class="font-semibold">${userName(o.user_id)}</span>
        <span class="text-[10px] px-2 py-0.5 rounded-full ${statusMap[o.status].cls}">${statusMap[o.status].label}</span>
      </div>
      <div class="text-sm text-gray-200 mb-1">${o.menu_type === 'その他' ? ('その他：' + (o.custom_menu_type || '')) : o.menu_type}</div>
      <div class="text-xs text-gray-400 mb-3">${o.note || '(メモなし)'}</div>
      <div class="text-[10px] text-gray-500 mb-3">申請日: ${o.date}</div>
      ${o.status === 'pending' ? `
        <div class="flex gap-2">
          <button onclick="updateOptinStatus('${o.id}', 'approved')" class="flex-1 bg-green-600 hover:bg-green-700 rounded-lg py-1.5 text-xs font-semibold"><i class="fa-solid fa-check mr-1"></i>承認</button>
          <button onclick="updateOptinStatus('${o.id}', 'rejected')" class="flex-1 bg-gray-700 hover:bg-gray-600 rounded-lg py-1.5 text-xs font-semibold"><i class="fa-solid fa-xmark mr-1"></i>却下</button>
        </div>
      ` : ''}
    </div>
  `).join('');
...
function renderRehabView() {
  ...
  el.innerHTML = `
    ...
    <p class="text-gray-400 text-sm mb-6">リハビリ中の選手のフェーズ・痛み報告状況を確認します</p>
  ...
  if (rehabPlayers.length === 0) {
    cardsEl.innerHTML = '<p class="text-gray-500 text-sm">現在、リハビリ中の選手はいません</p>';
    return;
  }
      ...
      <div class="bg-[#1f2229] rounded-2xl p-5">
        <div class="flex items-center justify-between mb-3">
          <div class="flex items-center gap-3">
            <div class="w-10 h-10 rounded-full bg-amber-500 flex items-center justify-center font-bold">${p.name.charAt(0)}</div>
            <div>
              <div class="font-semibold">${p.name}</div>
              <div class="text-xs text-amber-400">${p.rehab_phase_id}</div>
            </div>
          </div>
          <select onchange="updateRehabPhase('${p.id}', this.value)" class="bg-[#16181d] border border-gray-700 rounded-lg px-2 py-1 text-xs">
            <option value="phase_1" ${p.rehab_phase_id === 'phase_1' ? 'selected' : ''}>フェーズ1（急性期）</option>
            <option value="phase_2" ${p.rehab_phase_id === 'phase_2' ? 'selected' : ''}>フェーズ2（可動域改善）</option>
            <option value="phase_3" ${p.rehab_phase_id === 'phase_3' ? 'selected' : ''}>フェーズ3（筋力強化）</option>
            <option value="phase_4" ${p.rehab_phase_id === 'phase_4' ? 'selected' : ''}>フェーズ4（競技復帰）</option>
          </select>
        </div>
        <div class="text-xs text-gray-400 mb-2">専用メニュー: ${menu ? `${menu.exercise_name}（${menu.sets}×${menu.reps}, ${menu.note || ''}）` : '未設定'}</div>
        <div class="text-xs font-semibold text-gray-300 mb-1">直近の痛み報告</div>
        <div class="flex flex-col gap-1 max-h-32 overflow-y-auto no-scrollbar">
...
function renderReflectList() {
  ...
  const sorted = [...allReflections].sort((a, b) => new Date(b.date) - new Date(a.date) || (b.updated_label || '').localeCompare(a.updated_label || ''));
  if (sorted.length === 0) {
    el.innerHTML = '<p class="text-gray-500 text-sm col-span-2">まだ振り返りの投稿はありません</p>';
    return;
  }
  ...
  return `
    <div class="bg-[#1f2229] rounded-2xl p-4">
      <div class="flex items-center gap-2 mb-2">
        <div class="w-9 h-9 rounded-full bg-[var(--kuki-red)] flex items-center justify-center font-bold text-sm flex-shrink-0">${userName(r.user_id).charAt(0)}</div>
        <div>
          <div class="text-sm font-medium">${userName(r.user_id)}</div>
      ...
      <div class="flex flex-col gap-1.5 mb-2" id="dash-comments-${r.id}">
        ${comments.map(c => `
      ...
      <div class="flex gap-2">
        <input type="text" id="dash-comment-input-${r.id}" placeholder="コメントする..." class="flex-1 bg-[#16181d] border border-gray-700 rounded-lg px-3 py-1.5 text-xs">
        <button onclick="postDashComment('${r.id}')" class="bg-gray-700 hover:bg-[var(--kuki-red)] rounded-lg px-3 py-1.5 text-xs"><i class="fa-solid fa-paper-plane"></i></button>
      </div>
    </div>
  `;
}
...
let editingPlayerId = null; // 編集中の選手ID。nullなら新規登録モード

function renderPlayersView() {
  const el = document.getElementById('view-players');
  el.innerHTML = `
    <h1 class="font-display text-2xl font-bold mb-1">選手管理・成長確認</h1>
    <p class="text-gray-400 text-sm mb-6">選手の新規登録・編集・退部処理、および1RM推移の確認ができます（管理者専用）</p>

    <div class="bg-[#1f2229] rounded-2xl p-5 mb-6">
      <h2 class="font-semibold text-sm mb-4" id="player-form-title">選手の新規登録</h2>
      <div class="grid grid-cols-4 gap-3 mb-3">
        <div class="col-span-1">
          <label class="text-xs text-gray-400 block mb-1">氏名</label>
          <input id="pf-name" type="text" placeholder="山田 太郎" class="w-full bg-[#16181d] border border-[#2a2d36] rounded-lg px-3 py-2 text-sm">
        </div>
        <div class="col-span-1">
          <label class="text-xs text-gray-400 block mb-1">背番号</label>
          <input id="pf-jersey" type="number" min="0" placeholder="10" class="w-full bg-[#16181d] border border-[#2a2d36] rounded-lg px-3 py-2 text-sm">
        </div>
        <div class="col-span-1">
          <label class="text-xs text-gray-400 block mb-1">ポジション</label>
          <input id="pf-position" type="text" placeholder="FW / MF / DF / GK など" class="w-full bg-[#16181d] border border-[#2a2d36] rounded-lg px-3 py-2 text-sm">
        </div>
        <div class="col-span-1">
          <label class="text-xs text-gray-400 block mb-1">リハビリフェーズ</label>
          <select id="pf-rehab" class="w-full bg-[#16181d] border border-[#2a2d36] rounded-lg px-3 py-2 text-sm">
      ...
      <div class="flex gap-2">
        <button onclick="submitPlayerForm()" id="pf-submit-btn" class="px-4 py-2 rounded-lg bg-[var(--kuki-red)] text-white text-sm font-semibold"><i class="fa-solid fa-user-plus mr-1"></i>選手を登録</button>
        <button onclick="resetPlayerForm()" id="pf-cancel-btn" class="px-4 py-2 rounded-lg bg-[#2a2d36] text-gray-300 text-sm hidden">キャンセル</button>
      </div>
    </div>

    <div class="bg-[#1f2229] rounded-2xl p-5 mb-6">
      <h2 class="font-semibold text-sm mb-4">登録選手一覧（${allPlayers.length}名）</h2>
    ...
    <div class="grid grid-cols-3 gap-6">
      <div class="col-span-1 bg-[#1f2229] rounded-2xl p-4 max-h-[70vh] overflow-y-auto no-scrollbar">
        <div id="player-select-list" class="flex flex-col gap-1.5"></div>
      </div>
      <div class="col-span-2">
        <div class="bg-[#1f2229] rounded-2xl p-5 mb-5" id="player-detail-header">
          <p class="text-gray-500 text-sm">左のリストから選手を選択してください</p>
        </div>
        <div class="bg-[#1f2229] rounded-2xl p-5 hidden" id="player-detail-graph-wrap">
          <div class="flex gap-2 mb-4" id="player-ex-tabs"></div>
          <div style="height: 280px;"><canvas id="player-growth-canvas"></canvas></div>
        </div>
      </div>
    </div>
  `;
...
function renderPlayerManageTable() {
  ...
  tbody.innerHTML = allPlayers.map(p => `
    <tr class="border-b border-[#2a2d36]/60">
      <td class="py-2 pr-3">${p.jersey_number ?? '-'}</td>
      <td class="py-2 pr-3">${escapeHtml(p.name)}</td>
      <td class="py-2 pr-3">${escapeHtml(p.position || '-')}</td>
      <td class="py-2 pr-3">${p.rehab_phase_id ? '<span class="text-amber-400 text-xs">リハビリ中</span>' : '<span class="text-green-400 text-xs">在籍中</span>'}</td>
      <td class="py-2 pr-3 text-right whitespace-nowrap">
        <button onclick="startEditPlayer('${p.id}')" class="px-2 py-1 rounded-md bg-[#2a2d36] text-gray-200 text-xs mr-1"><i class="fa-solid fa-pen mr-1"></i>編集</button>
        <button onclick="deletePlayer('${p.id}')" class="px-2 py-1 rounded-md bg-red-900/40 text-red-300 text-xs"><i class="fa-solid fa-trash mr-1"></i>退部</button>
      </td>
    </tr>
  `).join('');
}
...
function resetPlayerForm() {
  ...
  document.getElementById('player-form-title').textContent = '選手の新規登録';
...
function startEditPlayer(userId) {
  ...
  document.getElementById('player-form-title').textContent = `選手情報の編集：${p.name}`;
...
async function submitPlayerForm() {
  ...
  try {
    ...
    allPlayers = allUsers.filter(u => u.role === 'player' && u.active !== false).sort((a,b) => (a.jersey_number||0) - (b.jersey_number||0));
    resetPlayerForm();
    renderPlayersView();
    renderOverview();
  } catch (e) {
...
function selectPlayerDetail(userId, btn) {
  document.querySelectorAll('.player-select-btn').forEach(b => b.classList.remove('bg-[var(--kuki-red)]', 'text-white'));
    ...
    <div class="flex items-center justify-between">
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          <div class="text-xs text-gray-400">${p.position || ''} ・ 背番号${p.jersey_number ?? '-'}${p.rehab_phase_id ? ' ・ <span class="text-amber-400">リハビリ中(' + p.rehab_phase_id + ')</span>' : ''}</div>
        </div>
      </div>
      <div class="text-right text-xs">
        <div class="mb-1">補食: ${cond?.nutrition_done ? '<span class="text-green-400">実施済</span>' : '<span class="text-gray-500">未実施</span>'}</div>
        <div>痛み: ${cond?.has_pain ? '<span class="text-red-400">あり</span>' : '<span class="text-gray-500">なし</span>'}</div>
      </div>
    </div>
  `;
...
function drawPlayerGrowth(userId, exercise, btn) {
  ...
  const records = allOneRm.filter(r => r.user_id === userId && r.exercise_name === exercise).sort((a,b) => new Date(a.date) - new Date(b.date));
  ...
  playerGrowthChart = new Chart(ctx, {
    ...
    data: {
      ...
      datasets: [{
        ...
        fill: true, tension: 0.3, pointRadius: 4, pointBackgroundColor: '#d61f2c'
      }]
    },
    options: {
